 				...
 				<view class="subPage--content--wrap">
 					<form @submit.prevent="formSave" class="content--wrap_form" :model="form">
 						...
 						<view class="content--wrap_form_li">
 							...
 							<view class="content--wrap_form_li_right">
 								<input class="text2 input" v-model="form.gj_fj" type="number" required="required" placeholder="填写罚金"/>
 							</view>
 						</view>					
 					</form>
 				</view>
 ...
 <script>
     ...
     export default {
 		...
 		methods:{			
			...
			// 保存进场准备检查表
 			onSave(){												
				this.$api.saveReadyEnterAreaForm(this.form).then(res =>{
					if(res.status === 200){
