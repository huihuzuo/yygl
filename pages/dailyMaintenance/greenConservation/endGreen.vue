	...
	<sub-page :title="title" :btnName="btnName" @onBtnClick="setting">		
		  <view slot="content" style="width:100%;height:100%;" class="mapContent">
			  <map 
				...
				:polyline="trajectory">
				<!-- #ifdef APP-PLUS -->	
				<cover-view class="btnMobile" @click="endClean">完成绿化</cover-view>
				<!-- #endif -->
			  </map>			  
			  <cover-view class="btn" @click="endClean">完成绿化</cover-view>				   
		  </view>
	</sub-page>	 
</template>
...
<script>
	...
	export default {
		...
		methods:{
			...
			addLhyh(){
				this.$api.insertLhyh(this.form).then(res =>{
					if(res.status === 200){
						console.log(res)
					}
				}).catch(e =>{
			...
			setting(){
				uni.navigateTo({
					url:'../greenConservation/greenSetting/greenSetting'
				})
			}
