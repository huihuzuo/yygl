				...
				<view class="subPage--content--wrap">
					<view  class="content--wrap_form">
						...
						<view class="content--wrap_form_li">
							...
							<view class="content--wrap_form_li_right">
								<picker class="text2" mode="date" :value="date" :start="startDate" :end="endDate" @change="bindjcrqChange" disabled>
						...
						<view class="content--wrap_form_li">
							...
							<view class="content--wrap_form_li_right">
								...
								<input class="text2 input"  type="text" v-model="detailObj.wyfj"/>
							</view>
						</view>
						<view class="content--wrap_form_li">
							<view class="content--wrap_form_li_left">
					...
					<view class="subPage--content-bottomhalf-innerwrap-li-content-detail">						
						...
						v-for="(mx,index) in detailObj.jczbmxb" 
						:key="index">
    ...
    export default {
		...
		onLoad:function(e){			
		 // 看一下传过来的是什么		 
		  ...
		  console.log(id)
		  // 请求详情页								  
			this.$api.getJczbById(id).then(res =>{
				if(res.data.code === 1){
					let resJczb = []
					resJczb = res.data.data
					resJczb.forEach((item) =>{
						this.detailObj = item
						this.mxList = item.jczbmxb
					})					
