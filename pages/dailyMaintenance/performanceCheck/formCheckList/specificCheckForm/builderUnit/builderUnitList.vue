					...
					<view class="subPage--content-bottomhalf-innerwrap-li">						
						...
						>
							...
							<view>
								...
								<text class="subPage--content-bottomhalf-innerwrap-li-content-text" >{{li.fjhj}}</text>
							</view>						
							<view>
							...
							<view class="subPage--content-bottomhalf-innerwrap-li-content-detail">
								...
								v-for="(mx,mxindex1) in li.lyjcmxb" 
								:key="mxindex1">
	...
	export default {
		...
		methods:{		
			...
			getSgdwAllList(){
				...
				this.$api.queryBuilderUnitList(this.form).then(res =>{					
					 console.log(res)
					if(res.data.code === 1){
						this.sgdwAllList = res.data.data.list
						this.total = res.data.data.total
						console.log(this.sgdwAllList)												
					}
					setTimeout(function () {
						uni.hideLoading();
					}, 500);
			...
			seeDetail:function(index){
				let vm = this	
				console.log(index)								
				let detailId = vm.sgdwAllList[index]['id']			
				...
				uni.navigateTo({
					url:'./detailBuilderUnit?id='+detailId
				})				
			},
			// 分页获取数据
			getPerpage(val){
			...
			deleLi(){
				...
				if(ids === ''){
					...
					this.$api.deleteLyjcByIds(ids).then(res =>{
						if(res.data.code === 1){
							uni.showModal({
								title: '是否确定删除?',								
		    ...
		    edit:function(index){
				...
				console.log(detailId)						
				uni.navigateTo({
					url:'./editBuilderUnit?id='+detailId
				})
			}						
		}
	}
