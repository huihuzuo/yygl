	...
	export default {
		...
		methods:{
			...
			getwxdjList(){
				...
				this.$api.getWxdjList(this.form).then(res =>{					
					 console.log(res)
					if(res.data.code === 1){
						this.wxdjAllList = res.data.data.list
						this.total = res.data.data.total
						console.log(this.wxdjAllList)												
					}
					setTimeout(function () {
						uni.hideLoading();
					}, 500);
			...
			seeDetail:function(index){
				let vm = this	
				console.log(index)								
				let detailId = vm.wxdjAllList[index]['djbh']			
				...
				uni.navigateTo({
					url:'../maintenanceRegistration/mrDetail/mrDetail?djbh='+detailId
				})				
			},
			// 分页获取数据
			getPerpage(val){
			...
			deleLi(){
				...
				if(djbhs === ''){
					...
					this.$api.delWxdjByDjbhs(djbhs).then(res =>{
						if(res.data.code === 1){
							uni.showModal({
								title: '是否确定删除?',								
...
// 			addCheck(){
// 				uni.navigateTo({
// 					url:'../maintenanceRegistration/addWxdj/addWxdj'
// 				})
// 			},				
			// 编辑操作
		    edit:function(index){
				...
				console.log(detailId)					
				uni.navigateTo({
					url:'../maintenanceRegistration/maintenanceRegistrationEdit/maintenanceRegistrationEdit?djbh='+detailId
				})
			}			
