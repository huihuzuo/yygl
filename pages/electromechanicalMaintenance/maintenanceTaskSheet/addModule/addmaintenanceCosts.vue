				...
				<view class="subPage--content--wrap">
					<view class="content--wrap_form">
						...
						<view class="content--wrap_form_li">
							...
							<view class="content--wrap_form_li_right">
								<picker class="text2" :value="sbmcIndex" @change="bindsbmcChange" :range="sbmcList">
									<view class="uni-input" v-if="sbmcIndex ===0 && sbmcCount ===0">请选择</view>
									<view class="uni-input" v-else>{{sbmcList[sbmcIndex]}}</view>
								</picker> 							 
						...
						<view class="content--wrap_form_li">
							...
							<view class="content--wrap_form_li_right">
								<picker class="text2" :value="sshtIndex" @change="bindsshtChange" :range="sshtList">
									<view class="uni-input" v-if="sshtIndex ===0 && sshtCount ===0">请选择</view>
									<view class="uni-input" v-else>{{sshtList[sshtIndex]}}</view>
								</picker> 							 
								<uni-icon class="icon" type="arrowright" size="20"></uni-icon>
							</view>
						</view>
						<view class="content--wrap_form_li">
							<view class="content--wrap_form_li_left">
								<text class="text1">对方单位</text>
							</view>
							<view class="content--wrap_form_li_right">
								<picker class="text2" :value="dfdwIndex" @change="binddfdwChange" :range="dfdwList">
									<view class="uni-input" v-if="dfdwIndex ===0 && dfdwCount ===0">请选择</view>
									<view class="uni-input" v-else>{{dfdwList[dfdwIndex]}}</view>
								</picker> 							 
								<uni-icon class="icon" type="arrowright" size="20"></uni-icon>
							</view>
						</view>
...
<script>
    ...
    export default {
		...
		methods:{			
			...
			binddfdwChange:function(e) {
				this.dfdwCount+=1
				this.dfdwIndex = e.target.value
				this.wxfy.dfdw = this.dfdwList[e.target.value]				
			},
			bindsshtChange:function(e) {
				this.sshtCount+=1
				this.sshtIndex = e.target.value
				this.wxfy.ssht = this.sshtList[e.target.value]				
			},
			getDate(type) {
				const date = new Date();			
			...
			// 获取所属合同名称
			getSsht(){				
				this.$api.queryForm(this.wxfy.ssht).then(res =>{					
					if(res.data.code ===1){						
						let resSsht = []
						resSsht = res.data.data
						resSsht.forEach(ssht =>{
							this.sshtList.push(ssht.htmc)
						})
					}
				}).catch(e =>{
			...
			// 获取对方单位
			getDfdw(){
				this.$api.getDfdwInfo().then(res =>{
					if(res.data.code ===1){
						let resDfdw = res.data.data
						resDfdw.forEach(dfdw =>{
							this.dfdwList.push(dfdw.dwmc)
						})
					}
				}).catch(e =>{
