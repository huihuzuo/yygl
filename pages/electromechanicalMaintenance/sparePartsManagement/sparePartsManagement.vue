				...
				<view class="subPage--content-bottomhalf-innerwrap">
					<view class="subPage--content-bottomhalf-innerwrap-li">						
						...
						>
							...
							<view>
								<text class="subPage--content-bottomhalf-innerwrap-li-content-text">需用人：</text>
								<text class="subPage--content-bottomhalf-innerwrap-li-content-text">{{li.xyr}}</text>
							</view>
							<view>
								<text class="subPage--content-bottomhalf-innerwrap-li-content-text">领用说明：</text>
								<text class="subPage--content-bottomhalf-innerwrap-li-content-text" >{{li.lysm}}</text>
							</view>
							<view>
								<text class="subPage--content-bottomhalf-innerwrap-li-content-text">附件：</text>
								<text class="subPage--content-bottomhalf-innerwrap-li-content-text" ></text>
							</view>
							<view>
	...
	export default {
		...
		methods:{		
			...
			getlysqList(){
				...
				this.$api.getLysqList(this.form).then(res =>{					
					 console.log(res)
					if(res.data.code === 1){
						this.bjglAllList = res.data.data.list
						this.total = res.data.data.total
						console.log(this.bjglAllList)												
					}
					setTimeout(function () {
						uni.hideLoading();
					}, 500);
			...
			seeDetail:function(index){
				let vm = this	
				console.log(index)								
				let detailId = vm.bjglAllList[index]['djbh']			
				...
				uni.navigateTo({
					url:'../sparePartsManagement/applyDetail/applyDetail?djbh='+detailId
				})				
			},
			// 分页获取数据
			getPerpage(val){
			...
			addCheck(){
				uni.navigateTo({
					url:'../sparePartsManagement/addSm/addSm'
				})				
			},
			// 勾选后删除操作
			deleLi(){
				...
				if(djbhs === ''){
					...
					this.$api.delLysqBydjbhs(djbhs).then(res =>{
						if(res.data.code === 1){
							uni.showModal({
								title: '是否确定删除?',								
		    ...
		    edit:function(index){
				...
				console.log(detailId)						
				uni.navigateTo({
					url:'../sparePartsManagement/editSm/editSm?djbh='+detailId
				})
			}			
